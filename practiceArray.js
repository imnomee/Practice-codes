const fruits = ["apple", "pear", "cherry"];

// for (let i = 0; i < fruits.length; i++) {
//     console.log(fruits[i]);
// }

// fruits.forEach(fruit => console.log(fruit));
const capFruits = [];

fruits.forEach(fruit => {
    capFruits.push(fruit.toUpperCase());
});

console.log(capFruits);

/// Calculate total price of array
const prices = [6.75, 3.1, 4.0, 8.12];
let totalPrice = 0;
prices.forEach(price => {
    totalPrice += price;
});
console.log(totalPrice);

//Log only the names starting with S
// const names = ["Selma", "Ted", "Mike", "Sam", "Sharon", "Marvin"];

// // names.forEach(name => {
// //     // if (name[0] == "S") { // Works the same
// //     if (name.charAt(0) == "S") {
// //         console.log(name);
// //     }
// // });

// names.forEach((name, index) => {
//     if (name.charAt(0) == "S") {
//         console.log(`${name} is found at index ${index}.`);
//     }
// });

const numbers = [1, 2, 3, 4, 5];
let total = 0;

// Write loop here:

numbers.forEach(number => (total += number));

// https://www.w3schools.com/jsref/jsref_substring.asp
//www.w3schools.com/jsref/jsref_slice_array.asp
const days = [
    "Sunday",
    "Monday",
    "Tuesday",
    "Wednesday",
    "Thursday",
    "Friday",
    "Saturday"
];
let dayAbbreviations = [];

// dayAbbreviations should be: ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa']
// Write your code below

days.forEach(day => {
    // dayAbbreviations.push(day.substring(0, 2)); //Works the same
    dayAbbreviations.push(day.slice(0, 2));
});

console.log(dayAbbreviations);

const stringPrices = ["5.47", "3.12", "8.00", "5.63", "10.70"];
let priceTotal = 0;

// priceTotal should be: 32.92
// Write your code below

// https://www.w3schools.com/jsref/jsref_parsefloat.asp
stringPrices.forEach(price => {
    priceTotal += parseFloat(price);
});
console.log(priceTotal);

const alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("");
let noel = [];

// noel should be: ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z']
// Write your code below

alphabet.forEach(character => {
    if (character != "L") {
        noel.push(character);
    }
});

console.log(noel);

/////////
const percentages = [34, 67, 12, 39, 90, 82, 22, 24, 99];
let upperRange = [];

// upperRange should be: [67, 90, 82, 99]
// Write your code below

percentages.forEach(value => {
    if (value > 50) {
        upperRange.push(value);
    }
});

console.log(upperRange);

//////////
const colors = [
    "#87E2D0",
    "#559F4D",
    "#FFE0F4",
    "#7E7E7E",
    "#FF2D2D",
    "#F7FFEC"
];
let filteredColors = [];

// filteredColors should be: ['#FFE0F4', '#FF2D2D', '#F7FFEC']
// Write your code below

colors.forEach(color => {
    if (color.charAt(1) == "F") {
        filteredColors.push(color);
    }
});

/////////
const months = ["january", "february", "march", "april", "may"];
let capitalizedMonths = [];

// filteredColors should be: ['JANUARY', 'FEBRUARY', 'MARCH', 'APRIL', 'MAY']
// Write your code below

months.forEach(month => {
    capitalizedMonths.push(month.toUpperCase());
});

////////////////

const years = [1989, 2015, 2000, 1999, 2013, 1973, 2012];
let displayYears;

// displayYears should be: ["2015 A.D.", "2013 A.D.", "2012 A.D."]
// Write your code below

displayYears = years.filter(year => year >= 2001).map(year => `${year} A.D.`);

console.log(displayYears);
